...
class CourseInfo {
  ...
  CourseInfo copyWith({
    ...
    return CourseInfo(
      ...
      videoPreview:
          ((videoPreview ?? '').isNotEmpty) ? videoPreview : this.videoPreview,
  ...
  CourseInfo.copyWithObject(CourseInfo json) {
    ...
    originalPrice = json.originalPrice ?? originalPrice;
  }
  ...
  CourseInfo.fromJson(Map<String, dynamic> json) {
    ...
    originalPrice = json['price_original'];
  }
...
/*    if((createdAt??"").isNotEmpty)
    ...
    if ((originalPrice != null)) {
      data['price_original'] = originalPrice ?? 0;
    }
    return data;
  }
  ...
  List<Subjects> getListSubjectAndLesson() {
    ...
    for (LessonInfo lessonInfo in lectures ?? []) {
      if ((subjects).isEmpty) {
        subjects
            .add(Subjects(subName: lessonInfo.subName, lectures: [lessonInfo]));
      } else {
        int index = 0;
        for (index = 0; index < (subjects ?? []).length; index++) {
          if (((subjects).elementAt(index).lectures ?? []).first.subName ==
              lessonInfo.subName) {
        ...
        if (index == (subjects).length) {
          /// truong hop cua co Subjects cho lecture nay
          subjects.add(
              Subjects(subName: lessonInfo.subName, lectures: [lessonInfo]));
        }
      }
    }
    return subjects;
