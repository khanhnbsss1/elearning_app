...
class Category1Courses extends ConsumerWidget {
  const Category1Courses({super.key, required this.category});

  final HomeCategory category;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final courses = ref.watch(category1CoursessProvider(category.id));
    return courses.when(
        data: (courses) {
          return Visibility(
            visible: true,
