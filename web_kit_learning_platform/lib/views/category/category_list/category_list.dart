...
import 'components/category_item_view.dart';
...
class _CategoryListPageState extends State<CategoryListPage> with SingleTickerProviderStateMixin, UIMixin {
 ...
 TextEditingController textEditingController = TextEditingController();
  ...
  Widget build(BuildContext context) {
    return PermissionPage(
      ...
      child: BlocProvider(
        create: (context) {
          return CategoryListBloc(CategoryListState())..add(CategoryListInitEvent());
        },
        child: BlocConsumer<CategoryListBloc, CategoryListState>(
          listener: (context, state) {
            switch (state.blocStatus) {
              ...
              case CategoryListStatus.onSelectTag:
                {
                }
                break;
              default:
                ...
                if(!myScreenMediaType.isMobile)
                  {
                    return Layout(
                        isScroll: false,
                        title: Center(
                          child: Text(L10nX.getStr.category_str,
  ...
  Widget buildLeftPage(
  {
    ...
    required CategoryListState state
  }
      ){
    return Container(
          ...
          Row(
            ...
            children: [
              FlutterCustomPagination(
                ...
                onPreviousPage: (p0) {
                  BlocProvider.of<CategoryListBloc>(context).add(CategoryListOnSearchByFilterEvent(searchCommonRequest: state.searchCommonRequest!.copyWith(pageNumber: p0 -1)));
                },
                ...
                child: Row(
                  children: [
                    ...
                    Visibility(
                      ...
                      child: InkWell(
                          onTap: () {
                            AddCategoryPage(tagPageAction: GradePageAction.create,).show(context, callBack: (p0) {
                              BlocProvider.of<CategoryListBloc>(context).add(CategoryListInitEvent());
                            },);
                          },
  ...
  Widget buildTagList({required CategoryListState state, required BuildContext context}){
    ...
    for (CategoryInfo lessonInfo in state.contentView ?? []) {
      listOfLesson.add(
        InkWell(
          onTap: () {
            BlocProvider.of<CategoryListBloc>(context).add(CategoryListOnSelectTagEvent(selectInfo: lessonInfo));
          },
          child: CategoryItemView(
            tagInfo: lessonInfo,
            
            onViewDetail: (p0) {
              AddCategoryPage(tagPageAction: GradePageAction.view,).show(context);
            },
            onEdit: (p0) {
              AddCategoryPage(info: p0,tagPageAction: GradePageAction.edit,).show(context, callBack: (p0) {
                BlocProvider.of<CategoryListBloc>(context).add(CategoryListInitEvent());
              },);
            },
            onDelete: (p0) {
              ConfirmPopupPage(
                ...
                onAccept: () {
                  BlocProvider.of<CategoryListBloc>(context).add(CategoryListOnDeleteTagEvent(selectInfo: p0));
                },
              ).show(context);
            },
          ),
    ...
    switch (state.blocStatus){
      ...
      case CategoryListStatus.initial:
        // TODO: Handle this case.
      case CategoryListStatus.onLoading:
        // TODO: Handle this case.
      case CategoryListStatus.onSearchByParams:
        // TODO: Handle this case.
        return Center(child: CircularProgressIndicator());
      case CategoryListStatus.onSelectTag:
      case CategoryListStatus.onLoadEnd:
        // TODO: Handle this case.
